...
import MyToken from "./contracts/MySCToken.json";
...
class App extends Component {
  ...
  getTotalTokenSupply = async () => {
    let totalTokenSupply = await this.tokenInstance.methods.totalSupply().call();
    //alert(totalTokenSupply);
    this.setState({totalTokens: totalTokenSupply});
  }
  ...
  listenToTokenTransfer = () => {
    ...
    this.tokenInstance.events.Transfer({ to: this.accounts[0] }).on("data", this.getTotalTokenSupply);
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <h2>Total Tokens in Circulation</h2>
        <h4>{ this.state.totalTokens } CAPPU TOKENS IN CIRCULATION</h4>
